...
export default function BlogListClient({ initialBlogPosts, initialPagination }) {
  const { router } = useAppContext();
  const { user } = useUser();

  const [blogPosts, setBlogPosts] = useState(initialBlogPosts);
  const [loading, setLoading] = useState(false);
  const [filters, setFilters] = useState({ category: '', careLevel: '', species: '' });
  const [pagination, setPagination] = useState(initialPagination);

  const fetchBlogPosts = async (page = 1) => {
    setLoading(true);
    const qp = new URLSearchParams({ page: page.toString(), limit: '9' });
    if (filters.category) qp.append('category', filters.category);
    if (filters.careLevel) qp.append('careLevel', filters.careLevel);
    if (filters.species) qp.append('species', filters.species);

    const res = await fetch(`/api/blog/list?${qp}`);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                >
                  ...
                  {[
                    'Care Guides', 'Species Profiles', 'Health & Veterinary',
                    'Feeding & Nutrition', 'Habitat Setup', 'Breeding',
                    'Beginner Tips', 'Equipment Reviews'
                  ].map(cat => (
                ...
                <article key={post._id} className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-6">
                    <div className="flex items-center mb-3">
                      <span className="inline-block bg-orange-100 text-orange-800 text-xs px-2 py-1 rounded-full mr-2">{post.category}</span>
                      {post.careLevel && (
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <time className="text-gray-500 text-sm">{formatDate(post.publishDate)}</time>
                      <Link href={`/blog/${post.slug}`} className="text-orange-600 hover:text-orange-700 font-medium text-sm">
                        Read More →
                      </Link>
                    </div>
